...
import {
  getSignedUrl, getSerie, updateSettingRead, getSettingRead
} from './read.service';

const api = express.Router();

api.get('/read/:serieId/:episodeId', skipGuestQuery(CheckAuth), async (req, res) => {
  try {
...
api.post('/setting-read', CheckAuth, async (req, res) => {
  try {
    const userId = req.userInfo && req.userInfo._id ? req.userInfo._id : '';
...
api.get('/setting-read', CheckAuth, async (req, res) => {
  try {
    const userId = req.userInfo && req.userInfo._id ? req.userInfo._id : '';
